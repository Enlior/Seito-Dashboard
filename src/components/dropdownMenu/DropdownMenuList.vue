        ...
        <el-checkbox-group @change="changeColumnShow" v-model="checkList">
            <VueDraggable handle=".handle" ref="el" :modelValue="list" @onSort="onUpdate" @update="onUpdate">
              <el-dropdown-item v-for="column in filterColumns"  :key="column.prop" style="display: flex;justify-content: space-between;"> 
                <el-checkbox :label="column.label?$t(column.label):$t('col.'+column.prop)" :value="column.prop" />
                <svg class="handle" xmlns="http://www.w3.org/2000/svg" width="0.88em" height="1em" viewBox="0 0 448 512"><path fill="currentColor" d="M0 96c0-17.7 14.3-32 32-32h384c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32m0 160c0-17.7 14.3-32 32-32h384c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32m448 160c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32s14.3-32 32-32h384c17.7 0 32 14.3 32 32"></path></svg>
              </el-dropdown-item>
            </VueDraggable>
        </el-checkbox-group>
      </el-dropdown-menu>
...
const initCheckList = (evt) => {
  ...
  for(let column of props.list) {
      if(column.isShow)
      checkList.value.push(column.prop)
  }
};
...
const changeColumnShow = ()=> {
  ...
  for(let column of props.list) {
    if(checkList.value.indexOf(column.prop )!=-1 ) {
      column.isShow = true;
    } else {
      column.isShow = false;
    }

      ...
      <el-dropdown-menu>
        <el-checkbox-group @change="changeColumnShow" v-model="checkList">
            <VueDraggable handle=".handle" ref="el" :modelValue="list" @onSort="onUpdate" @update="onUpdate">
              <el-dropdown-item v-for="column in filterColumns"  :key="column.name" style="display: flex;justify-content: space-between;"> 
                <el-checkbox :label="column.name?$t('col.'+column.name):$t(column.name)" :value="column.name" />
...
const initCheckList = () => {
  ...
  for(let column of props.list) {
      // if(column.isShow)
      checkList.value.push(column.name)
  }
};

const changeColumnShow = ()=> {
  for(let column of props.list) {
    if(checkList.value.indexOf(column.name)!=-1 ) {
      column.isShow = true;
    } else {
      column.isShow = false;
    }
